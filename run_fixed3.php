<?php
/**
 * WayWay - Fixed Test Runner v3
 * Key fix: After login, session is REGENERATED by Laravel.
 * The cookie jar must persist the NEW session cookie from the redirect response.
 * Solution: Follow the redirect after login to capture the new session.
 */
...
function req($method, $url, $data=null, $cookie='', $extraHeaders=[], $follow=false) {
    ...
    curl_close($ch);
    return ['code' => $code, 'body' => substr($resp, $hs), 'headers' => substr($resp, 0, $hs)];
}

function loginAndGetDashboard($baseUrl, $email, $password, $cookieFile, $dashboardPath) {
    if (file_exists($cookieFile)) unlink($cookieFile);

    // Step 1: GET login page - get CSRF + session cookie
    $r = req('GET', "$baseUrl/login", null, $cookieFile, ['Accept: text/html']);
    preg_match('/name="_token" value="([^"]+)"/', $r['body'], $m);
    $loginCsrf = $m[1] ?? '';
    echo "  [1] Login page: HTTP {$r['code']}, CSRF: ".($loginCsrf?substr($loginCsrf,0,12).'...':'MISSING')."\n";

    // Step 2: POST login - follow redirect to capture new session
    $r2 = req('POST', "$baseUrl/login", [
        '_token' => $loginCsrf,
        'email' => $email,
        'password' => $password,
    ], $cookieFile, [], true); // follow=true
    echo "  [2] Login POST (follow): HTTP {$r2['code']}\n";

    // Step 3: GET dashboard with the new session
    $r3 = req('GET', "$baseUrl$dashboardPath", null, $cookieFile, ['Accept: text/html']);
    preg_match('/name="csrf-token" content="([^"]+)"/', $r3['body'], $m2);
    $pageCsrf = $m2[1] ?? '';
    echo "  [3] Dashboard: HTTP {$r3['code']}, CSRF: ".($pageCsrf?substr($pageCsrf,0,12).'...':'MISSING')."\n\n";
...
echo "=== ADMIN ===\n";
$ac = 'C:/temp/adm4.txt';
[$adminCsrf, $pg, $dc] = loginAndGetDashboard($baseUrl, 'admin@wayway.com', 'Password123', $ac, '/admin/dashboard');
...
echo "\n=== PEMILIK WISATA ===\n";
$pc = 'C:/temp/pem4.txt';
[$pemilikCsrf, $pg, $dc] = loginAndGetDashboard($baseUrl, 'pemilik@test.com', 'Password123', $pc, '/pemilik/dashboard');
...
echo "\n=== ITINERARY DELETE FIX ===\n";
$wc3 = 'C:/temp/wis5.txt';
[$itinCsrf] = loginAndGetDashboard($baseUrl, 'wisatawan@test.com', 'Password123', $wc3, '/itinerary');
...
echo "\n=== TRAVEL AGENT ===\n";
$tac = 'C:/temp/ta4.txt';
[$agentCsrf, $pg, $dc] = loginAndGetDashboard($baseUrl, 'agent@test.com', 'Password123', $tac, '/travel-agent/dashboard');
...
echo "\n=== PARTNER FORM FIX ===\n";
$wc4 = 'C:/temp/wis6.txt';
[$partnerCsrf] = loginAndGetDashboard($baseUrl, 'wisatawan@test.com', 'Password123', $wc4, '/wisatawan/beranda');
...
echo "\n\n";
echo str_repeat('=',108)."\n";
echo "  GRAND TOTAL SUMMARY - WayWay API Collection (All Requests)\n";
echo str_repeat('=',108)."\n";
